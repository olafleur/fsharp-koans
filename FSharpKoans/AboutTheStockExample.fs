...
module ``about the stock example`` =
    ...
    let YouGotTheAnswerCorrect() =
        ...
        let getOpenValue chaine = 
            let tableau = splitCommas chaine
            System.Double.Parse(tableau.[1])

        let getCloseValue chaine = 
            let tableau = splitCommas chaine
            System.Double.Parse(tableau.[6])

        let getAbsValue chaine = abs(getOpenValue chaine - getCloseValue chaine)

        let ligne = stockData 
                    |> List.tail
                    |> List.maxBy getAbsValue

        let jour = splitCommas ligne

        AssertEquality "2012-03-13" jour.[0]
